...
6. **Ship.** Only after QA's verdict is "ready to ship" and the orchestrator has independently re-verified build/tests pass: create a new branch off the repository's default branch (currently `main`), commit the work using [Conventional Commits](https://www.conventionalcommits.org/) format (`feat:`, `fix:`, `docs:`, `test:`, `chore:`, etc. — scope commits to what actually changed; a bug-fix pass from the QA loop is its own `fix:` commit rather than being folded into the original `feat:` commit), push the branch, and open a pull request (`gh pr create` — this repo's remote is GitHub, so "PR" not "MR") summarizing what was built and pointing to `DESIGN_SPEC.md` and `TEST_REPORT.md` for full detail. Never commit or push directly to the default branch.
